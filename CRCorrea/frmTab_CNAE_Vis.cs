...
namespace CRCorrea
{
    public partial class frmTab_CNAE_Vis : Form
    {
        ...
        public DataTable CarregaGridTab_CNAE()
        {
            try
            {
                dtTab_CNAE = new DataTable();
                query = "select id, codigo, nome from tab_cnae order by codigo ";
            ...
            try
            {
                ...
                { 
                    if (MessageBox.Show("Deseja Realmente 'Excluir' o Registro: " +
                            dgvTab_CNAE_fil.Columns["CODIGO"].HeaderText + " " + dgvTab_CNAE_fil.CurrentRow.Cells["CODIGO"].Value, "Aplisoft",
                            ...
                            MessageBoxDefaultButton.Button2) == DialogResult.Yes)
                    {
                        ...
                        {
                            //EXCLUINDO                           
                            clsSelectInsertUpdateBLL.Delete(clsInfo.conexaosqldados, "TAB_CNAE_UF", " IDTABCNAE = " + dgvTab_CNAE_fil.CurrentRow.Cells["ID"].Value.ToString());
                            clsSelectInsertUpdateBLL.Delete(clsInfo.conexaosqldados, "TAB_CNAE", " ID = " + dgvTab_CNAE_fil.CurrentRow.Cells["ID"].Value.ToString());

                            dgvTab_CNAE_fil.Rows.Remove(dgvTab_CNAE_fil.CurrentRow);
